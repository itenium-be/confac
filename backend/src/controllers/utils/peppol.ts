import {
  AccountingCustomerParty, AccountingSupplierParty, AdditionalDocumentReference, AdditionalDocumentReferenceParams,
  ClassifiedTaxCategory, Contact, Country, InvoiceLine, Item, LegalMonetaryTotal, OrderReference, Party,
  PartyLegalEntity, PartyName, PartyTaxScheme, PaymentMeans, PostalAddress, Price, TaxCategory, TaxScheme,
...
export const createAccountingSupplierParty = (invoice: IInvoice, taxScheme: TaxScheme): AccountingSupplierParty => {
  ...
  const supplierEndpointID = new UdtIdentifier(companyNumber, {schemeID: companyNumberScheme});
  ...
  const supplierParty = new Party({
    partyNames: [
      new PartyName({name: new UdtName(invoice.your.name)}),
    ],
    ...
    partyTaxSchemes: [
      new PartyTaxScheme({companyID: cleanedVat, taxScheme}),
    ],
  });
...
export const createAccountingCustomerParty = (invoice: IInvoice, taxScheme: TaxScheme): AccountingCustomerParty => {
  const customerCountryAndCode = COUNTRY_CODES
    .find(codes => codes.country === invoice.client.country || codes.code === invoice.client.country);
  ...
  const customerParty = new Party({
    partyNames: [
      new PartyName({name: new UdtName(invoice.client.name)}),
    ],
...
type createTaxObjectsReturnType = {
 ...
 * and on the reason codes:
 * https://docs.peppol.eu/poacc/billing/3.0/syntax/ubl-invoice/cac-TaxTotal/cac-TaxSubtotal/cac-TaxCategory/cbc-TaxExemptionReasonCode/
 */
export const createTaxObjects = (invoice: IInvoice, taxScheme: TaxScheme): createTaxObjectsReturnType => {
  const customerCountryAndCode = COUNTRY_CODES
    .find(codes => codes.country === invoice.client.country || codes.code === invoice.client.country);
...
export const createPaymentMeans = (invoice: IInvoice): PaymentMeans => {
  ...
  const paymentMeans = new PaymentMeans({
    ...
    payeeFinancialAccount: new PayeeFinancialAccount({
      id: iban,
      financialInstitutioBranch: new FinancialInstitutionBranch({id: invoice.your.bic}),
    }),
  });
...
export const createOrderReference = (invoice: IInvoice): OrderReference => {
  ...
  const orderReference = new OrderReference({id: orderRef});
...
export const createInvoiceLine = (
  ...
  const unitCode = UNIT_CODES.find(uc => uc.unit === line.type);
  const invoiceLine = new InvoiceLine({
    ...
    item: new Item({
      ...
      sellersItemIdentification: new SellersItemIdentification({id: `${invoice.number}-${index + 1}`}),

...
export const invoices = (state: InvoiceModel[] = [], action: Action): InvoiceModel[] => {
  switch (action.type) {
    ...
    case ACTION_TYPES.MODELS_UPDATED: {
      const toUpdate = action.payload.filter((x: {type: string; model: InvoiceModel}) => x.type === 'invoice' && !!x.model);
      const removeIds = toUpdate.map((x: {model: InvoiceModel}) => x.model._id);
      const newState = state.filter(model => !removeIds.includes(model._id));
      toUpdate.forEach((model: {model: InvoiceModel}) => newState.push(mapInvoice(model.model)));
      return newState;
    }
